...
def _extract_features_from_url(u: str) -> Dict[str, object]:
    ...
    feats = {
        ...
        "nb_external_redirection": 0,  # page fetch needed
    ...
    # page / registry placeholders (kept 0 to avoid network/HTML fetches)
    feats.update({
...
def _align_to_expected(df_feat: pd.DataFrame,
    ...
    """
    Align to model's expected columns and ENFORCE dtypes:
    - numeric -> float (coerce errors to NaN then fill 0.0)
    - categorical -> string
    """
    ...
    for c in categorical_cols:
        # Use object dtype; ensure string (avoid NA -> "nan")
...
st.caption(
    ...
    "aligns to the model’s expected columns (dtype-safe), and predicts phishing probability."
)
