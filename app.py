...
def _detect_pos_idx_by_metadata_or_names(pipe, meta) -> int | None:
    ...
    if classes_:
        ...
        # If names look like ['legitimate','phishing'] or ['0','1'] the above would catch it.
    return None  # unknown

def _infer_pos_idx_with_probe(pipe, meta, numeric_cols, categorical_cols) -> int:
    """
    If class names aren't helpful, probe the model with a tiny synthetic set:
    pick the column where phishing-like URLs have higher mean prob than legit-like.
    """
    phishing_like = [
    ...
    df_probe = _featurize_urls(phishing_like + legit_like)
    X_probe, _ = _align_to_expected(df_probe, numeric_cols, categorical_cols)
    probs = pipe.predict_proba(X_probe)
    if probs.shape[1] != 2:
        return 1  # fallback for non-binary

    n_ph = len(phishing_like)
    ph_mean = probs[:n_ph].mean(axis=0)   # mean per column on phishing-like
    lg_mean = probs[n_ph:].mean(axis=0)   # mean per column on legit-like
    # choose the column with the biggest separation (phishing higher, legit lower)
    deltas = (ph_mean - lg_mean)
    return int(np.argmax(deltas))

def _resolve_positive_index(pipe, meta, numeric_cols, categorical_cols) -> int:
    idx = _detect_pos_idx_by_metadata_or_names(pipe, meta)
    if idx is not None:
        return idx
    return _infer_pos_idx_with_probe(pipe, meta, numeric_cols, categorical_cols)
...
if "attempted_disk_load" not in st.session_state:
    ...
    try:
        ...
        st.session_state.pos_idx = _resolve_positive_index(pipe, meta, numeric_cols, categorical_cols)
...
st.caption(
    ...
    "auto-detects the positive class (phishing) internally, and predicts probabilities."
)
        ...
        if not urls:
            ...
            if expected > 0 and (expected - filled) / expected < 0.6:
                st.warning(
                    ...
                    "If results look one-sided, consider moving your full feature engineering into the saved pipeline."
                )

            probs_all = st.session_state.pipe.predict_proba(X_aligned)
            if probs_all.shape[1] == 2:
                pos_col = st.session_state.pos_idx
            else:
                # non-binary fallback
            ...
            # Optional skew notice
            if probs_all.shape[1] == 2 and (np.mean(probs) > 0.95 or np.mean(probs) < 0.05):
                st.info("Auto-selected positive class via probe. If outputs still look wrong, the issue is likely feature mismatch, not class mapping.")
