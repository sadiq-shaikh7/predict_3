...
def is_valid_url(u):
    try:
        ...
        return bool(p.hostname and "." in p.hostname)
    except: return False

def _featurize_urls(urls):
    return URLFeatureizer().transform(pd.DataFrame({"url":urls}))
...
st.caption("Paste URLs below. Invalid or malformed URLs will be marked as 'invalid'.")
