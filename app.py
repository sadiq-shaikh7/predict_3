...
def _split_host(host: str) -> Dict[str, str]:
    """Very light split: [subdomains].[sld].[tld]"""
    h = (host or "").lower()
...
def _brand_hits(host: str, path: str) -> Dict[str, int]:
    s = (host + "/" + path).lower()
    # super small brand list; extend as needed
    ...
    return {
        ...
        "brand_in_subdomain": int(any(b in host.lower().split(".")[:-2] for b in brands)) if host else 0,
        "brand_in_path": int(any(b in path.lower() for b in brands)) if path else 0,
    }
...
def _extract_features_from_url(u: str) -> Dict[str, object]:
    ...
    feats = {
        ...
        "nb_dslash": url_text.count("//") - 1,  # minus the scheme
        ...
        "nb_external_redirection": 0,  # needs HTML fetch; leave 0
        "char_repeat": int(bool(re.search(r"(.)\1{3,}", url_text))),  # any char repeated >=4
    ...
    # hyperlink/css/media redirection/error features require page fetch & parsing → set 0
    page_related_zeros = {
    ...
    feats.update(page_related_zeros)

    # whois / age / traffic / dns / index features → set 0 (no network calls at inference)
    registry_zeros = {
    ...
    feats.update(registry_zeros)

    # keep raw stringy bits (in case encoders expect them)
    feats.update({
        "url": url_text,
        "hostname": host,
...
def _featurize_urls(urls: List[str]) -> pd.DataFrame:
    rows = [_extract_features_from_url(u) for u in urls]
...
st.caption(
    "Paste one URL per line. The app computes rich URL features to match the training schema, "
    "aligns to the model’s expected columns, and predicts phishing likelihood."
)
        ...
        if not urls:
            ...
            if expected > 0 and (expected - filled) / expected < 0.6:
                st.warning(
                    ...
                    "Predictions may skew legitimate. Consider moving full feature engineering into the saved pipeline."
                )

            probs_all = st.session_state.pipe.predict_proba(X_aligned)
            probs = probs_all[:, st.session_state.pos_idx]
