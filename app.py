...
class URLFeatureizer(BaseEstimator, TransformerMixin):
    ...
    def _extract_row(self,url_text:str)->Dict[str,Any]:
        url_text=(url_text or "").strip(); p=urlparse(url_text if "://" in url_text else "http://"+url_text)
...
# register shim so joblib can resolve url_features.URLFeatureizer
...
def _get_classes(pipe):
    if hasattr(pipe,"classes_"): return list(pipe.classes_)
    if hasattr(pipe,"named_steps"):
        est=list(pipe.named_steps.values())[-1]
        if hasattr(est,"classes_"): return list(est.classes_)
    return None
...
def _resolve_positive_index(pipe, meta, probe_df: pd.DataFrame) -> int:
    classes=_get_classes(pipe); target=(meta or {}).get("positive_class")
    if classes:
        if target in classes: return classes.index(target)
        for name in ("phishing","malicious","1",1,True):
            if name in classes: return classes.index(name)
    probs=pipe.predict_proba(probe_df)
    if probs.shape[1]==2:
        n=len(probe_df)//2; delta=probs[:n].mean(axis=0)-probs[n:].mean(axis=0)
        return int(np.argmax(delta))
    return 1 if probs.shape[1]>1 else 0

# ---------- Legacy support: external featurize + align ----------
def _featurize_urls(urls: List[str]) -> pd.DataFrame:
    # Use the same logic as URLFeatureizer.transform but stand-alone
    rows=[]
    for u in urls:
        url_text=(u or "").strip()
...
# Try predict with raw URL first; if model complains about missing columns, do legacy path.
def _predict_proba_hybrid(pipe, meta, urls: List[str]) -> Tuple[np.ndarray, Dict[str,Any]]:
    ...
    try:
        ...
        if not (numeric_cols or categorical_cols):
            raise RuntimeError("Loaded model expects engineered columns but metadata lacks numeric/categorical column lists.") from e
...
# ---------------- Helper: positive class index ----------------
def _resolve_pos_idx_once():
    if "pos_idx" in st.session_state: return
    pipe = st.session_state.get("pipe"); meta = st.session_state.get("meta")
    if pipe is None: return
    # use probe over the hybrid predictor so it works for both pipeline types
    probe_urls = _probe_df()["url"].tolist()
    probs, info = _predict_proba_hybrid(pipe, meta, probe_urls)
    if probs.shape[1]==2:
        n=len(probe_urls)//2; delta=probs[:n].mean(axis=0)-probs[n:].mean(axis=0)
        st.session_state.pos_idx = int(np.argmax(delta))
    else:
        st.session_state.pos_idx = 1 if probs.shape[1]>1 else 0

# ---------------- Main UI ----------------
st.title("🔍 Phishing URL Detector")
st.caption("Paste one URL per line. The app supports BOTH: a unified pipeline (raw URL input) and older legacy models (expects engineered columns).")

if "pipe" not in st.session_state:
    st.error("Model not loaded. Add artifacts to your repo OR upload them via the sidebar.")
else:
    _resolve_pos_idx_once()
    ...
    if st.button("Predict"):
        ...
        if not urls:
            ...
            mode = info.get("mode","?")
            if mode == "legacy(aligned_features)":
                missing_cols = info.get("missing_filled", [])
                st.info(f"Legacy model detected. Aligned to expected columns. Filled missing: {missing_cols or '[]'}")

            pos_col = st.session_state.get("pos_idx", 1 if probs_all.shape[1]>1 else 0)
            ...
            st.success(f"Mode: {mode}. Predicted {(out['prediction']=='phishing').sum()} phishing out of {len(out)} rows.")
            if show_probs:
